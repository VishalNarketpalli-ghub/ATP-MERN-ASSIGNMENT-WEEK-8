...
UserApp.get("/users/:id", async (req, res) => {
    try {
        ...
        let user = await UserTypeModel.findOne({ _id: uid, status: true })
...
UserApp.patch("/users/:id", async (req, res) => {
    let uid = req.params.id
    let user = await UserTypeModel.findByIdAndUpdate(uid, { $set: { status: true } }, { new: true })
    if (!user) return res.status(404).json({ message: "User Not Found" })
...
UserApp.put("/users/:id", async (req, res) => {
    try {
        let uid = req.params.id
        let updatedUser = await UserTypeModel.findByIdAndUpdate(uid, { $set: req.body }, { new: true, runValidators: true })
